...
class Blurplefy(commands.Cog):
    ...
    async def countdown(self, ctx):
        ...
        timeleft = datetime.datetime(now.year, 5, 13) + datetime.timedelta(hours=7) - discord.utils.utcnow()
        discord_years = now.year - 2015
        if timeleft.total_seconds() < 0:
            timeleft = (
                datetime.datetime((now.year + 1), 5, 13) + datetime.timedelta(hours=7) - discord.utils.utcnow()
            )
